...
- [ ] **Wire tier-gate feedback (spec §5):** `MiningController` publishes `MiningBlockedEvent { Asteroid, RequiredTier }` but no production subscriber exists yet, so a tier-gated mining attempt currently fails silently. When the higher-tier asteroid/drone SOs are authored (above), add a HUD subscriber (e.g. in `HUDController`) that surfaces "Requires a Tier {RequiredTier} drone." This is a no-op until tiers > 1 exist, which is why it's deferred here rather than fixed in code.
- [ ] **Server-deploy hardening (batch with the Cloud Code deploy):** wrap all 6 M6 functions (`AcquireDrone`, `UnlockDroneSlot`, `UpgradeDrone`, `SetActiveDrone`, `SellMinerals`, `ValidateMining`) in the `try/catch { logger.error(...); throw }` pattern the other `ServerCode/` functions use; align `loadFleet`'s empty default (`drones:[]`) to seed Scout like `GetBootstrapState` (removes a launch-ordering foot-gun); prune the dead `FLEET_KEY`/`DRONE_TIERS` constants in `AcquireDrone.js`. (Do NOT reorder `SellMinerals.js`'s decrement-then-grant sequence — the current order is the exploit-safe one.)
